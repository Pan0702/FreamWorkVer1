...
bool Window::Create(const wchar_t* title, uint32_t width, uint32_t height)
{
    ...
    // 呼び出し側が指定した描画領域サイズを、Win32のRECTに変換する。
    RECT rect = {
        0, 0,
        static_cast<LONG>(width), static_cast<LONG>(height)
    };

    // CreateWindowExに渡すため、外枠とタイトルバー込みのサイズへ補正する。
    AdjustWindowRect(&rect, WS_OVERLAPPEDWINDOW, FALSE);

    int window_width = rect.right - rect.left;
    int window_height = rect.bottom - rect.top;
    
    // WndProcでこのC++オブジェクトを取り出せるよう、最後の引数にthisを渡す。
...
bool Window::ProcessMessages() const
{
    ...
    {
        // WM_QUITを受け取ったら、アプリケーションのメインループを終了する。
...
LRESULT Window::WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    // 最初の生成メッセージには、CreateWindowExで渡したthisが入っている。
    if (msg == WM_NCCREATE)
    {
        auto* create_struct = reinterpret_cast<CREATESTRUCTW*>(lParam);
        window = static_cast<Window*>(create_struct->lpCreateParams);
        // 次回以降のメッセージで使えるよう、HWNDにthisを保存する。
