...
bool ShadowRenderer::Initialize(ID3D12Device* device, DescriptorHeap* srv_heap)
{
    ...
    // skmesh
    {
        ...
        //スキンメッシュの頂点レイアウトで、深度だけを書き込むＰＳＯを作成する。
    ...
    if (!shadow_map_.Initialize(device, kShadowMapSize, srv_heap))
    {
        return false;
    }
    return true;
                                       ...
                                       const SkinnedMeshRenderer* skinned, const InstancedMeshRenderer* instance, const FrameSnap& frame) const
{
    ...
    //SRVとして読み込んだShadowMapを深度への書き込み先として使うためDEPTH_WRITEへ遷移する。
    //  ShadowMap::Create でD3D12_RESOURCE_STATE_DEPTH_WRITE 初期化なので、
    //  厳密には StateBefore = PIXEL_SHADER_RESOURCE が初回と合わないかも
    ...
    // シャドウマップ全体に描画するため、viewportとscissorをテクスチャサイズに合わせる
