...
bool IndexBuffer::Initialize(ID3D12Device* device, const void* index_data, uint32_t total_size, DXGI_FORMAT format)
{
    ...
    errno_t err = memcpy_s(mapped, total_size, index_data, total_size);
    ...
    const uint32_t element_size =
        (format == DXGI_FORMAT_R16_UINT) ? sizeof(uint16_t) : sizeof(uint32_t);
    index_count_ = total_size / element_size;
